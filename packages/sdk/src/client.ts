 ...
 */
function normalizeAndAssertNonZeroHash(value: unknown): `0x${string}` {
  ...
  if (body.length === 0 || /^0+$/.test(body)) {
    throw new TypeError(
      'resultHash must not be the zero hash (the contract rejects ZeroHashRejected; pre-check saves a round-trip)',
    );
  }
...
function isInjectedSigner(s: unknown): s is InjectedSignerWithAddress {
  return (
    typeof s === 'object' &&
    s !== null &&
    typeof (s as { address?: unknown }).address === 'string' &&
    (s as { signer?: unknown }).signer != null
  );
}

function isKeyringPair(s: unknown): s is IKeyringPair {
  return (
    typeof s === 'object' &&
    s !== null &&
    typeof (s as { sign?: unknown }).sign === 'function' &&
    typeof (s as { address?: unknown }).address === 'string'
  );
}
...
export class BountyMeshClient {
  public readonly program: SailsProgram;
  private readonly signer: IKeyringPair | InjectedSignerWithAddress;
  private readonly events: SubscriptionManager;
  ...
  constructor(opts: BountyMeshClientOptions) {
    ...
    if (!isKeyringPair(opts.signer) && !isInjectedSigner(opts.signer)) {
      throw new TypeError(
        'signer must be an IKeyringPair (Node) or { address, signer } (browser injection)',
      );
    }
  ...
  async post(args: PostArgs): Promise<TxResult<{ bountyId: bigint }>> {
    ...
    if ('ok' in reply) {
      return {
        ok: true,
        value: { bountyId: BigInt(reply.ok as number | string | bigint) },
        txHash,
        blockHash,
      };
    }
  ...
  async accept(id: bigint): Promise<TxResult<null>> {
   ...
   */
  async withdraw(id: bigint): Promise<TxResult<null>> {
  ...
  async onBountyPosted(
    filter: BountyPostedFilter | null,
    cb: (e: BountyPostedEvent) => void | Promise<void>,
  ): Promise<Unsubscribe> {
    return this.events.on('BountyPosted', filter, cb);
  }

  async onBountyClaimed(
    filter: BountyClaimedFilter | null,
    cb: (e: BountyClaimedEvent) => void | Promise<void>,
  ): Promise<Unsubscribe> {
    return this.events.on('BountyClaimed', filter, cb);
  }

  async onBountySubmitted(
    filter: BountySubmittedFilter | null,
    cb: (e: BountySubmittedEvent) => void | Promise<void>,
  ): Promise<Unsubscribe> {
    return this.events.on('BountySubmitted', filter, cb);
  }

  async onBountyAccepted(
    filter: BountyAcceptedFilter | null,
    cb: (e: BountyAcceptedEvent) => void | Promise<void>,
  ): Promise<Unsubscribe> {
    return this.events.on('BountyAccepted', filter, cb);
  }

  async onBountyWithdrawn(
    filter: BountyWithdrawnFilter | null,
