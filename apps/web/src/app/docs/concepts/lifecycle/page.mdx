...
Each terminal transition refunds the escrowed reward to the appropriate party via `CommandReply::with_value` (poster on Cancel/Revoke/TimedOut-before-Submit, retained for worker on TimedOut-after-Submit). See [refund correctness](/docs/concepts/anti-cheat#refund-correctness) for the caller-vs-target rule that governs these.
